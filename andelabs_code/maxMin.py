"""
 return an array containing the min and max numbers respectively
 max: maximum number
 min: minimum number
"""

def find_max_min(numlist=[]):
	max_min_list = [] # empty list to hold the min and max numbers
	if min(numlist) == max(numlist): # if max number is equal to min number
		max_min_list.append(max(numlist))
		return max_min_list
	else:  # min and max numbers exist
		max_min_list.append(min(numlist))
		max_min_list.append(max(numlist))
		return max_min_list
